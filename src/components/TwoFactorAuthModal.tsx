...
export const TwoFactorAuthModal: React.FC<TwoFactorAuthModalProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let errorMessage = t('auth.twoFactorAuth.invalidCode');
      let errorCode = null;
      ...
      toast({
        title: t('auth.twoFactorAuth.failedTitle'),
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-md" onInteractOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="text-2xl text-center">{t('auth.twoFactorAuth.title')}</DialogTitle>
          <DialogDescription className="text-center">
            <div className="flex flex-col items-center mt-2">
              <div className="bg-primary/10 p-3 rounded-full mb-4">
                <Shield className="h-8 w-8 text-primary" />
              </div>
              <p>{t('auth.twoFactorAuth.description')}</p>
              <p className="text-sm mt-2">{t('auth.twoFactorAuth.enterCode')}</p>
            </div>
          </DialogDescription>
        </DialogHeader>
